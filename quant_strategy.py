...
# %%
base_cols = ['회사명', '업종대', '업종소', '주가(원)', '시가총액(억)', 'PER', 'PBR', '연간배당률(%)']
#########################################################
# %%
def 시가총액하위20퍼센트(df):
...
def 매출액YOYQOQ영업이익YOYQOQ순이익YOYQOQ_0이상(df):
    ...
    df = df[df[f'지배순이익{YEAR}{QUATER}YOY'] >= 0]
    df = df[df[f'지배순이익{YEAR}{QUATER}QOQ'] >= 0]
...
def 신F스코어_3점이상(df):
    신F스코어컬럼 = ['F스코어순이익>0여부', 'F스코어영업활동현금>0여부', 'F스코어신주발행X여부']
...
소형주_df.loc[:, 'PBR역수']=소형주_df.loc[:,'자본총계']/소형주_df.loc[:,'시가총액(억)']
"""
def 신마법공식_소형주(df1, n):
    """ GP/A, PBR 사용
    """
    df1 = df1[df1['차입률(%)'] <= 200]
    df1 = 시가총액하위20퍼센트(df1)
    df1.loc[:, 'PBR역수'] = df1.loc[:,'자본총계']/df1.loc[:,'시가총액(억)']

    df1 = df1[(df1[['GP/A(%)','PBR역수']] != 0).all(axis=1)] #하나라도 0이면 제외
    df1['순위GP/A'] = df1['GP/A(%)'].rank(ascending=False)
...
# %%
def 소형주_밸류C(df1, n):  # 소형주 밸류C 시총 하위 20%에서 PFCR','PSR','POR','PER','분기PER','PBR', 'F스코어점수 (9점만점)' 종합 점수 높은순중에서 종가 낮은순 정렬
    """ FIXME : 한종목이 다르게 나온다. (이노인스트루먼트) 이유를 모르겠다.
    """
    df1 =시가총액하위20퍼센트(df1)

    cols = ['PFCR', 'PSR', 'POR', 'PER', '분기PER', 'PBR']
...
def 전종목_배당성장(df1, n):  # 전종목 배당성장 실적 상승 종목 중 배당률 정렬 (매우합격)
    df1 = 매출액YOYQOQ영업이익YOYQOQ순이익YOYQOQ_0이상(df1)
    df1 = 영업활동현금흐름_순이익지배_0이상(df1)
    _전종목_배당성장 = df1.sort_values(by='연간배당률(%)', ascending=False).head(n)
...
def 소형주_해자밸류(df1, n):  # 소형주 해자밸류 실적 상승 종목 중 슈퍼가치주 (매우합격)
    ...
    매출영업순이익성장 = df1[[f'매출액{YEAR}{QUATER}YOY', f'매출액{YEAR}{QUATER}QOQ',
                          ...
                          f'지배순이익{YEAR}{QUATER}YOY', f'지배순이익{YEAR}{QUATER}QOQ']]
    매출영업순이익성장종합랭킹 = 종합_랭킹_계산(매출영업순이익성장, True)
    df1 = df1.loc[매출영업순이익성장종합랭킹.index]

    슈퍼가치_컬럼추출 = df1[['PFCR', 'PSR', 'POR', 'PER', '분기PER', 'PBR']]
...
def 소형주_해자성장A(df1, n):  # 소형주 해자성장A 시총하위 종목중 이익률지표 성장률
    ...
    이익율성장율 = df1[['분기OPM증가율(%)','분기NPM증가율(%)','ROE증가율(%)','자본증가율(%)']]
...
# 중형주_성장B(df.copy(), 20)
# %%
def 중형주_밸류C(df1, n):  # 중형주 밸류C 시총 중위종목에서 PFCR','PSR','POR','PER','분기PER','PBR', 'F스코어점수 (9점만점)' 종합 점수 높은순중에서 종가 낮은순 정렬
    """FIXME : 한종목이 다르다. 이노인스트루먼트
    """
    df1 = 시가총액중형주(df1)

    슈퍼가치_컬럼추출 = df1[['PFCR', 'PSR', 'POR', 'PER', '분기PER', 'PBR']].copy()
